...
Purpose :
* Compliance - To force functions to adhere to the definitions set within an interface
    ```
...
1. Interacting with  contract:
    1. Deploy the contract: 
        ...
        ```
    2. Remove the contract deployment, but the sol file must be compiled 
    3. Note : If using an interface, we can remove all the code, except the interface and still be able to interact with the contract
    ...
    #TODO - Research more about interfaces, cover payable section of video
    getting error when trying to send 